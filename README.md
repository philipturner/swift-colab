...
Now, the real action begins. The TensorFlow Swift packages takes 3 minutes to compile on Google Colab, which sounds worse than it is. This is a one-time cost because of Swift-Colab 2.0, and it will build instantaneously the next time you do `Runtime > Restart runtime`. Grab a cup of coffee or read a Medium article while it compiles. If you accidentally close the browser tab with S4TF loaded, try salvaging it with `Runtime > Manage sessions`.
