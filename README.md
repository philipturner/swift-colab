...
To install a Swift package, type `%install`, followed by a Swift 4.2-style package declaration within single quotes. After that, type the modules you want to compile. Before importing any module via a Swift `import` statement, execute its `%install` command. You can install packages in any cell, which was not possible in [swift-jupyter](https://github.com/google.swift-jupyter).
